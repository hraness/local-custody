...
export type LocalCustodyFallbackNotice = Readonly<{
  /**
   * Stable surface tag identifying the engine path that fell back, for example
   * "local-custody-rust-fallback".
   */
  tag: string;
  /**
   * Bounded reason class, for example "load-failed", "spawn-failed",
   * "protocol-failed", or "unsupported-input".
   */
  reason: string;
  /** Optional bounded input classification, for example "object" or "pipe". */
  inputClass?: string;
}>;
 ...
 */
export function emitLocalCustodyFallback(notice: LocalCustodyFallbackNotice): void {
